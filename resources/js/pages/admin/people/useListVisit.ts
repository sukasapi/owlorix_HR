...
/**
 * Tracks GET visits to one list page (filters, search, pagination) so the page can show
 * a loading line and a retry panel instead of a frozen table or the default error modal.
 */
export function useListVisit(pathname: string) {
    ...
    useEffect(() => {
        const isListVisit = (visit: { method: string; url: URL; prefetch?: boolean }) =>
            visit.method === 'get' && visit.url.pathname === pathname && !visit.prefetch;
